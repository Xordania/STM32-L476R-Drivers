...
void I2C1_EV_IRQHandler(void){
	...
	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_RXIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_RXNE);

	//1. Handle interrupt generated by RXNE event
	if(event_flag && control_bit){
		//RXNE flag is set

		*pI2C1HandleLink->pRxBuffer = pI2C1HandleLink->pI2Cx->RXDR;
		pI2C1HandleLink->pRxBuffer++;
		pI2C1HandleLink->nReceived++;
		...
		if(pI2C1HandleLink->nReceived == pI2C1HandleLink->RxLen){
			if(pI2C1HandleLink->Sr == 0){
				I2C_ApplicationEventCallback(pI2C1HandleLink, I2C_EV_RX_CMPLT);
			}
		}
	}

	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_TXIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_TXIS);
	...
	if(event_flag && control_bit){
		...
		pI2C1HandleLink->pI2Cx->TXDR = *pI2C1HandleLink->pTxBuffer;
		pI2C1HandleLink->pTxBuffer++;
		pI2C1HandleLink->nTransmitted++;

		// If NBYTES have been transmitted and a repeated start is not required let the
		//application know the transmit has finished successfully
		if(pI2C1HandleLink->nTransmitted == pI2C1HandleLink->TxLen){
			if(pI2C1HandleLink->Sr == 0){
				I2C_ApplicationEventCallback(pI2C1HandleLink, I2C_EV_TX_CMPLT);
			}
		}
	...
	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_STOPIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_STOPF);
	...
	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_TCIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_TCR);
	...
	if(event_flag && control_bit){
		...
		if(pI2C1HandleLink->TxRxState == I2C_BUSY_IN_TX){
			I2C_ApplicationEventCallback(pI2C1HandleLink, I2C_EV_TX_CMPLT);
		}else{
			I2C_ApplicationEventCallback(pI2C1HandleLink, I2C_EV_RX_CMPLT);
		}
	...
	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_TCIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_TC);
	...
	if(event_flag && control_bit){
		...
		if(pI2C1HandleLink->nTransmitted == pI2C1HandleLink->TxLen){
			pI2C1HandleLink->TxRxState = I2C_NOT_BUSY;

			// Let the application know that the transmission has been complete
			I2C_ApplicationEventCallback(pI2C1HandleLink, I2C_EV_TX_CMPLT);
		}else if(pI2C1HandleLink->nReceived == pI2C1HandleLink->RxLen){
			pI2C1HandleLink->TxRxState = I2C_NOT_BUSY;
	...
	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_ADDRIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_ADDR);

	//6. Handle interrupt generated by ADDRIE event
	//NOTE: This will only wake up the device from Stop mode if the I2C instance
	//suports the Wakeup from Stop mode feature (all for Stop Mode 1. #
	//Only I2C3 for Stop Mode 2)
	if(event_flag && control_bit){
		//ADDR flag is set
	}

	event_flag = pI2C1HandleLink->pI2Cx->CR1 & (1 << I2Cx_CR1_NACKIE);
	control_bit = pI2C1HandleLink->pI2Cx->ISR & (1 << I2Cx_ISR_NACKF);
